...
import { TERMS_DEFAULT_CONTENT, TERMS_DEFAULT_TITLE } from '@/lib/static-pages';
...
export default function TermsOfServicePage() {
  ...
  const [title, setTitle] = useState(TERMS_DEFAULT_TITLE);
  const [content, setContent] = useState(TERMS_DEFAULT_CONTENT);
  const [lastUpdated, setLastUpdated] = useState<Date>(new Date());
  const [editing, setEditing] = useState(false);
  const [tempTitle, setTempTitle] = useState(TERMS_DEFAULT_TITLE);
  const [tempContent, setTempContent] = useState(TERMS_DEFAULT_CONTENT);
  ...
  const saveChanges = async () => {
    try {
      setSaving(true);
      const pageRef = doc(db, 'pages', 'terms');
