...
const PromotionScreen: React.FC = () => {
  ...
  const promotionServicesItems = pageAreas?.promotionServices?.items || [];
  const featuresItems = pageAreas?.features?.items || [];

  const renderPromotionService = (service: any, index: number) => (
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        const areas = await fetchServicePageAreas('promotion');
        setPageAreas(areas);
      } catch (error) {
        if (__DEV__) {
          console.log(
            'Warning: unable to load promotion page data for mobile, using defaults:',
            error,
          );
        }
      }
  ...
  const promotionServicesTitle = pageAreas?.promotionServices?.title;
  const promotionServicesDescription = pageAreas?.promotionServices?.description;
  ...
  const headerTitle = mobileAppData?.categories?.category6?.title;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{promotionServicesTitle}</Text>
          {promotionServicesDescription ? (
            <Text style={styles.sectionDescription}>{promotionServicesDescription}</Text>
          ) : null}
          {promotionServicesItems.length > 0 ? (
          <View style={styles.servicesContainer}>
              {promotionServicesItems.map((service: any, index: number) => renderPromotionService(service, index))}
          </View>
          ) : null}
        </View>
