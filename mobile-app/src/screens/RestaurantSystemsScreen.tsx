...
const RestaurantSystemsScreen: React.FC = () => {
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        const areas = await fetchServicePageAreas('restaurant-systems');
        setPageAreas(areas);
      } catch (error) {
        if (__DEV__) {
          console.log(
            'Warning: unable to load restaurant-systems page data for mobile, using defaults:',
            error,
          );
        }
      }
  ...
  const headerTitle = mobileAppData?.categories?.category7?.title;
