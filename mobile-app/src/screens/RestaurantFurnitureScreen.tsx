...
const RestaurantFurnitureScreen: React.FC = () => {
  ...
  const renderFurnitureCategory = (category: any, index: number) => (
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        const areas = await fetchServicePageAreas('restaurant-furniture');
        setPageAreas(areas);
      } catch (error) {
        if (__DEV__) {
          console.log(
            'Warning: unable to load restaurant-furniture page data for mobile, using defaults:',
            error,
          );
        }
      }
  ...
  const headerTitle = mobileAppData?.categories?.category5?.title;
        ...
        <View style={styles.section}>
            ...
            {categoriesItems.length > 0 ? (
              <View style={styles.categoriesContainer}>
                {categoriesItems.map((category: any, index: number) => renderFurnitureCategory(category, index))}
          </View>
            ) : null}
          </View>
        ) : null}
